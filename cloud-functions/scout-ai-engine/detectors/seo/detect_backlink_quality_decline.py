"""'backlink_quality_decline' Detector"""
...
def detect_backlink_quality_decline(organization_id: str) -> list:
    ...
    logger.info("🔍 Running 'backlink_quality_decline' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "seo_opportunity", "type": "backlink_quality_decline", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "seo_keyword",
                "title": f"SEO opportunity: 'backlink_quality_decline'", "description": f"Keyword '{row.canonical_entity_id}' detected for 'backlink_quality_decline' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'backlink_quality_decline' opportunities")
