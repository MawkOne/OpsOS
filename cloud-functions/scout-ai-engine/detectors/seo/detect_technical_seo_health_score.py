"""'technical_seo_health_score' Detector"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "seo_opportunity", "type": "technical_seo_health_score", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "seo_keyword",
                "title": f"SEO opportunity: 'technical_seo_health_score'", "description": f"Keyword '{row.canonical_entity_id}' detected for 'technical_seo_health_score' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'technical_seo_health_score' opportunities")
