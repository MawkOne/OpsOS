"""'content_freshness_decay' Detector"""
...
def detect_content_freshness_decay(organization_id: str) -> list:
    ...
    logger.info("🔍 Running 'content_freshness_decay' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "seo_opportunity", "type": "content_freshness_decay", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "seo_keyword",
                "title": f"SEO opportunity: 'content_freshness_decay'", "description": f"Keyword '{row.canonical_entity_id}' detected for 'content_freshness_decay' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'content_freshness_decay' opportunities")
