"""'internal_link_opportunities' Detector"""
...
def detect_internal_link_opportunities(organization_id: str) -> list:
    ...
    logger.info("🔍 Running 'internal_link_opportunities' detector...")
    opportunities = []
    query = f"""
    ...
    WHERE m.organization_id = @org_id AND m.date >= DATE_SUB(CURRENT_DATE(), INTERVAL 90 DAY)
      AND e.entity_type = 'seo_keyword' AND impressions > 100
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "seo_opportunity", "type": "internal_link_opportunities", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "seo_keyword",
                "title": f"SEO opportunity: 'internal_link_opportunities'", "description": f"Keyword '{row.canonical_entity_id}' detected for 'internal_link_opportunities' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'internal_link_opportunities' opportunities")
