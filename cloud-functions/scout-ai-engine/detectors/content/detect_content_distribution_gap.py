"""'content_distribution_gap' Detector"""
...
def detect_content_distribution_gap(organization_id: str) -> list:
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "content_optimization", "type": "content_distribution_gap", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "content",
                "title": f"Content opportunity: 'content_distribution_gap'", "description": f"Content '{row.canonical_entity_id}' detected for 'content_distribution_gap' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'content_distribution_gap' opportunities")
