"""'publishing_volume_gap' Detector"""
...
def detect_publishing_volume_gap(organization_id: str) -> list:
    ...
    logger.info("🔍 Running 'publishing_volume_gap' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "content_optimization", "type": "publishing_volume_gap", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "content",
                "title": f"Content opportunity: 'publishing_volume_gap'", "description": f"Content '{row.canonical_entity_id}' detected for 'publishing_volume_gap' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'publishing_volume_gap' opportunities")
