"""'content_format_winners' Detector"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "content_optimization", "type": "content_format_winners", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "content",
                "title": f"Content opportunity: 'content_format_winners'", "description": f"Content '{row.canonical_entity_id}' detected for 'content_format_winners' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'content_format_winners' opportunities")
