    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                ...
                "title": f"Content opportunity: 'content_to_lead_attribution'", "description": f"Content '{row.entity_name}' detected for 'content_to_lead_attribution' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'content_to_lead_attribution' opportunities")
