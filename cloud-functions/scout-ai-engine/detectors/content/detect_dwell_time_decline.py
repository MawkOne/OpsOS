...
def detect_dwell_time_decline(organization_id: str) -> list:
    ...
    logger.info("🔍 Running 'dwell_time_decline' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "content_optimization", "type": "dwell_time_decline", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "content",
                "title": f"Content opportunity: 'dwell_time_decline'", "description": f"Content '{row.canonical_entity_id}' detected for 'dwell_time_decline' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'dwell_time_decline' opportunities")
