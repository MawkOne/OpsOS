"""'engagement_rate_decline' Detector"""
...
def detect_engagement_rate_decline(organization_id: str) -> list:
    ...
    logger.info("🔍 Running 'engagement_rate_decline' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "content_optimization", "type": "engagement_rate_decline", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "content",
                "title": f"Content opportunity: 'engagement_rate_decline'", "description": f"Content '{row.canonical_entity_id}' detected for 'engagement_rate_decline' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'engagement_rate_decline' opportunities")
