"""'alert_fatigue_detection' Detector"""
...
def detect_alert_fatigue_detection(organization_id: str) -> list:
    bq_client = bigquery.Client()
    logger.info("🔍 Running 'alert_fatigue_detection' detector...")
    opportunities = []
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "alert_fatigue_detection", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'alert_fatigue_detection'", "description": f"Data quality issue detected: 'alert_fatigue_detection'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'alert_fatigue_detection' issues")
