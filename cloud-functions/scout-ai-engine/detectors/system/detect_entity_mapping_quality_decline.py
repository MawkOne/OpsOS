"""'entity_mapping_quality_decline' Detector"""
...
def detect_entity_mapping_quality_decline(organization_id: str) -> list:
    bq_client = bigquery.Client()
    logger.info("🔍 Running 'entity_mapping_quality_decline' detector...")
    opportunities = []
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "entity_mapping_quality_decline", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'entity_mapping_quality_decline'", "description": f"Data quality issue detected: 'entity_mapping_quality_decline'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'entity_mapping_quality_decline' issues")
