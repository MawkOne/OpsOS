...
def detect_missing_data_gaps(organization_id: str) -> list:
    logger.info("🔍 Running 'missing_data_gaps' detector...")
    opportunities = []
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "missing_data_gaps", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'missing_data_gaps'", "description": f"Data quality issue detected: 'missing_data_gaps'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'missing_data_gaps' issues")
