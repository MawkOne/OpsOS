"""'detector_performance_monitoring' Detector"""
...
def detect_detector_performance_monitoring(organization_id: str) -> list:
    logger.info("🔍 Running 'detector_performance_monitoring' detector...")
    opportunities = []
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "detector_performance_monitoring", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'detector_performance_monitoring'", "description": f"Data quality issue detected: 'detector_performance_monitoring'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'detector_performance_monitoring' issues")
