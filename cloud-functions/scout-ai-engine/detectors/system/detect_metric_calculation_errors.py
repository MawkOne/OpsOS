"""'metric_calculation_errors' Detector"""
...
def detect_metric_calculation_errors(organization_id: str) -> list:
    logger.info("🔍 Running 'metric_calculation_errors' detector...")
    opportunities = []
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "metric_calculation_errors", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'metric_calculation_errors'", "description": f"Data quality issue detected: 'metric_calculation_errors'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'metric_calculation_errors' issues")
