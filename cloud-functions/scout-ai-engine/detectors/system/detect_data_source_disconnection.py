"""'data_source_disconnection' Detector"""
...
def detect_data_source_disconnection(organization_id: str) -> list:
    logger.info("🔍 Running 'data_source_disconnection' detector...")
    opportunities = []
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "data_source_disconnection", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'data_source_disconnection'", "description": f"Data quality issue detected: 'data_source_disconnection'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'data_source_disconnection' issues")
