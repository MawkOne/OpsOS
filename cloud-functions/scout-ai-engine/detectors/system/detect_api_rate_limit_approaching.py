"""'api_rate_limit_approaching' Detector"""
...
def detect_api_rate_limit_approaching(organization_id: str) -> list:
    logger.info("🔍 Running 'api_rate_limit_approaching' detector...")
    opportunities = []
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "api_rate_limit_approaching", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'api_rate_limit_approaching'", "description": f"Data quality issue detected: 'api_rate_limit_approaching'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'api_rate_limit_approaching' issues")
