...
def detect_bigquery_cost_spike(organization_id: str) -> list:
    ...
    try:
        ...
        for row in bq_client.query(query, job_config=job_config).result():
            ...
            if data_age_hours > 24:  # Data older than 24 hours
                opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                    "category": "system_health", "type": "bigquery_cost_spike", "priority": "high", "status": "new",
                    "entity_id": "system", "entity_type": "data_pipeline",
                    "title": f"System alert: 'bigquery_cost_spike'", "description": f"Data quality issue detected: 'bigquery_cost_spike'",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'bigquery_cost_spike' issues")
