...
def detect_page_speed_decline(organization_id: str) -> list:
    ...
    logger.info("🔍 Running 'page_speed_decline' detector...")
    opportunities = []
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "page_optimization", "type": "page_speed_decline", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "page",
                "title": f"Optimization opportunity: 'page_speed_decline'", "description": f"Page '{row.canonical_entity_id}' ready for 'page_speed_decline' analysis",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'page_speed_decline' opportunities")
