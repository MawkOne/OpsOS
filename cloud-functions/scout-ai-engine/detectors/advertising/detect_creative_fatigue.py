...
def detect_creative_fatigue(organization_id: str) -> list:
    bq_client = bigquery.Client()
    logger.info("🔍 Running 'creative_fatigue' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "advertising_optimization", "type": "creative_fatigue", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "ad_campaign",
                "title": f"Ad opportunity: 'creative_fatigue'", "description": f"Campaign '{row.canonical_entity_id}' detected for 'creative_fatigue' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'creative_fatigue' opportunities")
