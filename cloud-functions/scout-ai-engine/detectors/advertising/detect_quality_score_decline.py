"""'quality_score_decline' Detector"""
...
def detect_quality_score_decline(organization_id: str) -> list:
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "advertising_optimization", "type": "quality_score_decline", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "ad_campaign",
                "title": f"Ad opportunity: 'quality_score_decline'", "description": f"Campaign '{row.canonical_entity_id}' detected for 'quality_score_decline' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'quality_score_decline' opportunities")
