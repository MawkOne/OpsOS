...
def detect_impression_share_loss(organization_id: str) -> list:
    bq_client = bigquery.Client()
    logger.info("🔍 Running 'impression_share_loss' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                ...
                "title": f"Ad opportunity: 'impression_share_loss'", "description": f"Campaign '{row.entity_name}' detected for 'impression_share_loss' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'impression_share_loss' opportunities")
