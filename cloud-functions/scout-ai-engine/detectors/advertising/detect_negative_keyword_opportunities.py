"""'negative_keyword_opportunities' Detector"""
...
def detect_negative_keyword_opportunities(organization_id: str) -> list:
    bq_client = bigquery.Client()
    logger.info("🔍 Running 'negative_keyword_opportunities' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                ...
                "category": "advertising_optimization", "type": "negative_keyword_opportunities", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "ad_campaign",
                "title": f"Ad opportunity: 'negative_keyword_opportunities'", "description": f"Campaign '{row.canonical_entity_id}' detected for 'negative_keyword_opportunities' optimization",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'negative_keyword_opportunities' opportunities")
