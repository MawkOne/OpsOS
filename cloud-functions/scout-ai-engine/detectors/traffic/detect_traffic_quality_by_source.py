"""'traffic_quality_by_source' Detector"""
...
def detect_traffic_quality_by_source(organization_id: str) -> list:
    logger.info("🔍 Running 'traffic_quality_by_source' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "traffic_optimization", "type": "traffic_quality_by_source", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "traffic_source",
                "title": f"Traffic opportunity: 'traffic_quality_by_source'", "description": f"Source '{row.entity_name}' detected for 'traffic_quality_by_source' analysis",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'traffic_quality_by_source' opportunities")
