"""'traffic_source_disappearance' Detector"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "traffic_optimization", "type": "traffic_source_disappearance", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "traffic_source",
                "title": f"Traffic opportunity: 'traffic_source_disappearance'", "description": f"Source '{row.entity_name}' detected for 'traffic_source_disappearance' analysis",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'traffic_source_disappearance' opportunities")
