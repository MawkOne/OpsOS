"""'channel_mix_optimization' Detector"""
...
def detect_channel_mix_optimization(organization_id: str) -> list:
    logger.info("🔍 Running 'channel_mix_optimization' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "traffic_optimization", "type": "channel_mix_optimization", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "traffic_source",
                "title": f"Traffic opportunity: 'channel_mix_optimization'", "description": f"Source '{row.entity_name}' detected for 'channel_mix_optimization' analysis",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'channel_mix_optimization' opportunities")
