"""'attribution_model_comparison' Detector"""
...
def detect_attribution_model_comparison(organization_id: str) -> list:
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "traffic_optimization", "type": "attribution_model_comparison", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "traffic_source",
                "title": f"Traffic opportunity: 'attribution_model_comparison'", "description": f"Source '{row.entity_name}' detected for 'attribution_model_comparison' analysis",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'attribution_model_comparison' opportunities")
