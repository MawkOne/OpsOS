"""'cross_device_journey_issues' Detector"""
...
def detect_cross_device_journey_issues(organization_id: str) -> list:
    logger.info("🔍 Running 'cross_device_journey_issues' detector...")
    opportunities = []
    query = f"""
    ...
    GROUP BY canonical_entity_id, entity_name, source
    LIMIT 20
    """
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "traffic_optimization", "type": "cross_device_journey_issues", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "traffic_source",
                "title": f"Traffic opportunity: 'cross_device_journey_issues'", "description": f"Source '{row.entity_name}' detected for 'cross_device_journey_issues' analysis",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'cross_device_journey_issues' opportunities")
