"""'multitouch_path_issues' Detector"""
...
def detect_multitouch_path_issues(organization_id: str) -> list:
    logger.info("🔍 Running 'multitouch_path_issues' detector...")
    opportunities = []
    query = f"""
    ...
    try:
        for row in bq_client.query(query, job_config=job_config).result():
            opportunities.append({"id": str(uuid.uuid4()), "organization_id": organization_id, "detected_at": datetime.utcnow().isoformat(),
                "category": "traffic_optimization", "type": "multitouch_path_issues", "priority": "medium", "status": "new",
                "entity_id": row.canonical_entity_id, "entity_type": "traffic_source",
                "title": f"Traffic opportunity: 'multitouch_path_issues'", "description": f"Source '{row.entity_name}' detected for 'multitouch_path_issues' analysis",
        ...
        if opportunities: logger.info(f"✅ Found {len(opportunities)} 'multitouch_path_issues' opportunities")
