# Scout AI Detector Roadmap

**Last Updated:** January 27, 2026  
**Progress:** 47 of 117 detectors complete (40%)

---

## 📊 Quick Stats

- ✅ **Complete:** 47 detectors
- ⏳ **To Do:** 70 detectors
- 📈 **Total Goal:** 117 detectors

---

## Detectors Checklist

### 📧 Email Marketing (13/13 complete) ✅

- [x] Email Engagement Drop
- [x] High Opens, Low Clicks
- [x] Email Trends (Multi-Timeframe)
- [x] Bounce Rate Spike
- [x] Spam Complaint Spike
- [x] List Health Decline
- [x] Click-to-Open Rate Decline
- [x] Optimal Frequency Deviation
- [x] Email Volume Gap
- [x] Revenue Per Subscriber Decline
- [x] Device/Client Performance Gap
- [x] A/B Test Recommendations
- [x] List Segmentation Opportunities

### 💵 Revenue & Metrics (8/19 complete)

- [x] Revenue Anomaly
- [x] Metric Anomalies
- [x] Revenue Trends (Multi-Timeframe)
- [x] AOV Decline
- [x] Payment Failure Spike
- [x] New Customer Decline
- [x] Discount Cannibalization
- [x] Seasonality Deviation
- [ ] MRR/ARR Tracking
- [ ] Transaction/Refund Anomalies
- [ ] Forecast Deviation
- [ ] Unit Economics Dashboard
- [ ] Growth Velocity Trends
- [ ] Cohort Performance Trends
- [ ] Customer Churn Spike
- [ ] LTV:CAC Ratio Decline
- [ ] Revenue Concentration Risk
- [ ] Pricing Opportunity Analysis
- [ ] Expansion Revenue Gap

### 📄 Pages & CRO (10/18 complete)

- [x] High Traffic, Low Conversion
- [x] Page Engagement Decay
- [x] Scale Winners
- [x] Fix Losers
- [x] Scale Winners (Multi-Timeframe)
- [x] Form Abandonment Spike
- [x] Cart Abandonment Increase
- [x] Page Error Rate Spike
- [x] Micro-Conversion Drop
- [x] Exit Rate Increase
- [ ] Mobile vs Desktop CVR Gap
- [ ] A/B Test Opportunities
- [ ] Page Speed Decline
- [ ] Conversion Funnel Drop-Off
- [ ] CTA Performance Analysis
- [ ] Video Engagement Gap
- [ ] Social Proof Opportunities
- [ ] Trust Signal Gaps

### 🚦 Traffic Sources (7/16 complete)

- [x] Cross-Channel Gaps
- [x] Declining Performers
- [x] Declining Performers (Multi-Timeframe)
- [x] Bot/Spam Traffic Spike
- [x] Traffic Spike Quality Check
- [x] UTM Parameter Gaps
- [x] Referral Opportunities
- [ ] Traffic Source Disappearance
- [ ] Channel Dependency Risk
- [ ] Revenue by Channel Attribution
- [ ] Multi-Touch Path Issues
- [ ] Traffic Quality by Source
- [ ] CAC by Channel
- [ ] Channel Mix Optimization
- [ ] Attribution Model Comparison
- [ ] Cross-Device Journey Issues

### 🔍 SEO (4/12 complete)

- [x] Keyword Cannibalization
- [x] Striking Distance Keywords
- [x] Rank Drops
- [x] Rank Trends (Multi-Timeframe)
- [ ] Rank Volatility (Daily)
- [ ] Content Freshness Decay
- [ ] Technical SEO Health Score
- [ ] Internal Link Opportunities
- [ ] Featured Snippet Opportunities
- [ ] Backlink Quality Decline
- [ ] Core Web Vitals Failing
- [ ] Schema Markup Gaps

### 💰 Advertising (3/13 complete)

- [x] Cost Inefficiency
- [x] Paid Waste
- [x] Campaign Trends (Multi-Timeframe)
- [ ] Ad Retargeting Gap
- [ ] Creative Fatigue
- [ ] Audience Saturation (Proxy)
- [ ] Device/Geo Optimization Gaps
- [ ] Quality Score Decline
- [ ] Impression Share Loss
- [ ] Ad Schedule Optimization
- [ ] Negative Keyword Opportunities
- [ ] Competitor Activity Alerts
- [ ] Landing Page Relevance Gap

### ✍️ Content (2/11 complete)

- [x] Content Decay
- [x] Content Decay (Multi-Timeframe)
- [ ] Publishing Volume Gap
- [ ] Content-to-Lead Attribution
- [ ] Topic Gap Analysis
- [ ] Content Format Winners
- [ ] Engagement Rate Decline
- [ ] Dwell Time Decline
- [ ] Content Pillar Opportunities
- [ ] Republishing Opportunities
- [ ] Content Distribution Gap

### 🏗️ System & Data Quality (0/15 complete)

- [ ] Data Freshness Issues
- [ ] Entity Mapping Quality Decline
- [ ] Data Source Disconnection
- [ ] Metric Calculation Errors
- [ ] BigQuery Cost Spike
- [ ] API Rate Limit Approaching
- [ ] Duplicate Data Detection
- [ ] Missing Data Gaps
- [ ] Schema Drift Detection
- [ ] Data Quality Score
- [ ] Alert Fatigue Detection
- [ ] False Positive Rate
- [ ] Opportunity Resolution Tracking
- [ ] Detector Performance Monitoring
- [ ] Cross-Detector Correlation

---

## 📝 Notes

- All checked detectors are deployed to `scout-ai-engine` Cloud Function
- This roadmap is the single source of truth
- To update: Change `- [ ]` to `- [x]` when a detector is built and deployed
- Verify deployed detectors: Check `cloud-functions/scout-ai-engine/detectors/*.py`
