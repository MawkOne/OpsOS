...
export async function GET(request: NextRequest) {
  ...
  const eventFilter = searchParams.get('event') || ''; // Optional: filter to specific event
  ...
  try {
    ...
    const eventsData: Record<string, Record<string, any>> = {};
    ...
    for (const month of months) {
      ...
      if (!response.ok) {
        console.error(`Failed to fetch events for ${month.key}:`, await response.text());
        continue;
      }
      ...
      if (data.rows) {
        for (const row of data.rows) {
          ...
          eventsData[eventId].months[month.key] = {
    ...
    const result = Object.values(eventsData);
