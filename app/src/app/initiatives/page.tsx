...
import { Initiative, statusConfig, calculateWaterlinePosition, calculateInitiativeCosts, initiativeCategories, InitiativeStatus, InitiativeCategory } from "@/types/initiatives";
      ...
      {showDetailModal && selectedInitiative && (
        <InitiativeDetailModal
          ...
          onUpdate={async (updates) => {
            await updateDoc(doc(db, "initiatives", selectedInitiative.id), {
              ...updates,
...
async function seedInitiatives(organizationId: string) {
  ...
  const initiativesData = [
    // MARKETING
    {
      ...
      priority: "high",
      status: "in-progress" as InitiativeStatus,
...
function InitiativeDetailModal({ 
  ...
  onClose: () => void; 
  onUpdate: (updates: Partial<Initiative>) => Promise<void>;
}) {
  const [activeTab, setActiveTab] = useState<"overview" | "forecast" | "scenarios" | "montecarlo">("overview");
  const [saving, setSaving] = useState(false);
  
  // Forecast state
  const [forecastEnabled, setForecastEnabled] = useState(initiative.forecast?.enabled || false);
  const [monthlyRevenue, setMonthlyRevenue] = useState<number[]>(
    initiative.forecast?.monthlyRevenue || Array(12).fill(0)
  );
  
  // Scenario state
  const [scenarios, setScenarios] = useState(initiative.scenarios || {
  ...
  // Monte Carlo state
  const [monteCarlo, setMonteCarlo] = useState(initiative.monteCarlo || {
    simulations: 10000,
    expectedValue: 0,
    standardDeviation: 0,
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-6xl max-h-[90vh] overflow-hidden rounded-2xl shadow-2xl flex flex-col bg-[#0a0a0a] border border-[#2a2a2a]"
      >
        {/* Header */}
        <div className="flex items-start justify-between p-6 border-b border-[#2a2a2a] bg-[#0f0f0f]">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">{initiative.name}</h2>
                <span 
            ...
            <p className="text-sm text-gray-400 mt-2">{initiative.description}</p>
          </div>
          <button
            onClick={onClose}
            className="ml-4 p-2 rounded-lg hover:bg-[#1a1a1a] transition-colors text-gray-400 hover:text-white"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-[#2a2a2a] bg-[#0f0f0f] px-6">
          {tabs.map((tab) => (
            <button
              key={tab.id}
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-3 p-6 border-t border-[#2a2a2a] bg-[#0f0f0f]">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg text-sm font-medium text-gray-400 hover:text-white hover:bg-[#1a1a1a] transition-colors"
          >
            Close
          </button>
