...
import { TrendingUp, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
...
export default function PublishingPerformancePage() {
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        opp.title?.toLowerCase().includes('content') && (opp.title?.toLowerCase().includes('publish') || opp.title?.toLowerCase().includes('volume') || opp.title?.toLowerCase().includes('freshness'))
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Publishing & Performance</h1>
          </div>
          <p className="text-gray-400">
            Monitor content publishing volume and performance
          </p>
        </div>
