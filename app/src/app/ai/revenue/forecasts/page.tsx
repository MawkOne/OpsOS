...
import { LineChart, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
...
export default function AnomaliesForecastsPage() {
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        opp.title?.toLowerCase().includes('revenue') && (opp.title?.toLowerCase().includes('anomaly') || opp.title?.toLowerCase().includes('forecast') || opp.title?.toLowerCase().includes('trend'))
      );
  ...
  return (
    <AppLayout module="ai">
      <div className="space-y-6">
        <div>
          <Link href="/ai/revenue" className="inline-flex items-center gap-2 text-gray-400 hover:text-white mb-4 transition-colors">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Anomalies & Forecasts</h1>
          </div>
          <p className="text-gray-400">
            Monitor revenue anomalies and forecast deviations
          </p>
        </div>
