...
export default function EfficiencyAnalysisPage() {
  ...
  async function fetchData() {
    try {
      ...
      const filtered = data.opportunities?.filter(
        (opp: any) => opp.category === "cost_inefficiency" || opp.type?.includes("waste") || opp.description?.toLowerCase().includes("roas")
      ) || [];
  ...
  return (
    <AppLayout
      ...
      subtitle="Cost inefficiency, waste, and ROAS deterioration"
    >
      {/* What This Detects */}
      <Card className="glass mb-8">
        <h2 className="text-xl font-bold mb-4" style={{ color: "var(--foreground)" }}>What Efficiency Analysis Finds</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Cost Inefficiency</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>High spend with low returns</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Paid Waste</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Wasted ad spend on underperforming campaigns</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>ROAS Decline</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Return on ad spend deteriorating over time</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Budget Optimization</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Opportunities to reallocate spend</p>
            </div>
          </div>
        </div>
      </Card>
