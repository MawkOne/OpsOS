...
import { Activity, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        (opp.entity_type === 'page' || opp.title?.toLowerCase().includes('page')) && (opp.title?.toLowerCase().includes('mobile') || opp.title?.toLowerCase().includes('desktop') || opp.title?.toLowerCase().includes('device'))
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Mobile vs Desktop</h1>
          </div>
