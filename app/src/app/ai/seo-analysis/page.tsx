...
export default function SeoAnalysisPage() {
  ...
  async function fetchData() {
    try {
      ...
      const filtered = data.opportunities?.filter(
        (opp: any) => opp.category === "seo_issues" || opp.type?.includes("seo") || opp.type?.includes("rank")
      ) || [];
  ...
  return (
    <AppLayout
      title="SEO Analysis"
      subtitle="Rank drops, striking distance, and position trends"
    >
      {/* What This Detects */}
      <Card className="glass mb-8">
        <h2 className="text-xl font-bold mb-4" style={{ color: "var(--foreground)" }}>What SEO Analysis Finds</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Rank Drops</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Keywords losing position over time</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Striking Distance</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Keywords close to page 1</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Position Trends</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Multi-month ranking improvements/declines</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>SEO Opportunities</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Quick wins for better rankings</p>
            </div>
          </div>
        </div>
      </Card>
