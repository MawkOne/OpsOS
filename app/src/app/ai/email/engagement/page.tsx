...
import { Mail, Activity, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
...
export default function EmailEngagementPage() {
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      // Filter for email engagement opportunities
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        (opp.entity_type === 'email' || opp.title?.toLowerCase().includes('email')) &&
        (opp.title?.toLowerCase().includes('engagement') ||
         opp.title?.toLowerCase().includes('open') ||
         opp.title?.toLowerCase().includes('click') ||
         opp.title?.toLowerCase().includes('volume') ||
         opp.title?.toLowerCase().includes('sends'))
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Email Engagement & Volume</h1>
          </div>
          <p className="text-gray-400">
            Monitoring open rates, click rates, send volume, and engagement trends
          </p>
        </div>
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-400">
              ...
              <p>No engagement opportunities found</p>
            </div>
          ) : (
