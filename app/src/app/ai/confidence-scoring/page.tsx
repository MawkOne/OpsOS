...
export default function ConfidenceScoringPage() {
  ...
  async function fetchData() {
    try {
      ...
      const filtered = data.opportunities?.filter(
        (opp: any) => opp.confidence_score >= 0.85
      ) || [];
  ...
  return (
    <AppLayout
      ...
      subtitle="Reliability assessment, data volume, and statistical significance"
    >
      {/* What This Detects */}
      <Card className="glass mb-8">
        <h2 className="text-xl font-bold mb-4" style={{ color: "var(--foreground)" }}>What Confidence Scoring Finds</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>High Confidence</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Opportunities with 85%+ confidence</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Data Volume</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Sufficient data for reliable conclusions</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Statistical Significance</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Results unlikely due to chance</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Reliability Score</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>How much to trust each opportunity</p>
            </div>
          </div>
        </div>
      </Card>
