...
import { Link2, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        opp.title?.toLowerCase().includes('traffic') && (opp.title?.toLowerCase().includes('source') || opp.title?.toLowerCase().includes('quality') || opp.title?.toLowerCase().includes('disappear'))
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Source Analysis</h1>
          </div>
          <p className="text-gray-400">
            Monitor traffic sources, disappearances, and quality
          </p>
        </div>
