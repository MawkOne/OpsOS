...
import { PieChart, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
...
export default function ChannelMixCACPage() {
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        opp.title?.toLowerCase().includes('channel') || opp.title?.toLowerCase().includes('cac') || opp.title?.toLowerCase().includes('dependency')
      );
  ...
  return (
    <AppLayout module="ai">
      <div className="space-y-6">
        <div>
          <Link href="/ai/traffic" className="inline-flex items-center gap-2 text-gray-400 hover:text-white mb-4 transition-colors">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Channel Mix & CAC</h1>
          </div>
          <p className="text-gray-400">
            Analyze channel dependency, CAC, and mix optimization
          </p>
        </div>
