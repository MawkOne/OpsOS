...
import { TrendingUp, Search, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        (opp.entity_type === 'keyword' || opp.title?.toLowerCase().includes('seo') || opp.title?.toLowerCase().includes('search')) &&
        (opp.title?.toLowerCase().includes('rank') ||
         opp.title?.toLowerCase().includes('keyword') ||
         opp.title?.toLowerCase().includes('volatility') ||
         opp.title?.toLowerCase().includes('position'))
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Rankings & Keywords</h1>
          </div>
          <p className="text-gray-400">
            Monitor ranking changes, keyword performance, and volatility
          </p>
        </div>
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-400">
              ...
              <p>No ranking opportunities found</p>
            </div>
          ) : (
