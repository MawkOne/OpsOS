...
export default function EmailAnalysisPage() {
  ...
  async function fetchData() {
    try {
      ...
      const filtered = data.opportunities?.filter(
        (opp: any) => opp.entity_type === "email" || opp.category === "email_issues" || opp.type?.includes("email")
      ) || [];
  ...
  return (
    <AppLayout
      ...
      subtitle="Engagement trends, open/click patterns, and deliverability"
    >
      {/* What This Detects */}
      <Card className="glass mb-8">
        <h2 className="text-xl font-bold mb-4" style={{ color: "var(--foreground)" }}>What Email Analysis Finds</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Engagement Trends</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Open and click rates over time</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>High Opens Low Clicks</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Good subject lines, poor content</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Deliverability Issues</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Declining send success rates</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Campaign Performance</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Best and worst email campaigns</p>
            </div>
          </div>
        </div>
      </Card>
