...
import { DollarSign, Megaphone, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
...
export default function AdvertisingEfficiencyPage() {
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        (opp.entity_type === 'campaign' || opp.category === 'cost_inefficiency' || opp.title?.toLowerCase().includes('ad')) &&
        (opp.title?.toLowerCase().includes('efficiency') ||
         opp.title?.toLowerCase().includes('budget') ||
         opp.title?.toLowerCase().includes('spend') ||
         opp.title?.toLowerCase().includes('cac') ||
         opp.title?.toLowerCase().includes('roas') ||
         opp.title?.toLowerCase().includes('device') ||
         opp.title?.toLowerCase().includes('geo'))
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold">Budget & Efficiency</h1>
          </div>
          <p className="text-gray-400">
            Optimize ad spend, CAC, ROAS, and device/geo targeting
          </p>
        </div>
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-400">
              ...
              <p>No efficiency opportunities found</p>
            </div>
          ) : (
