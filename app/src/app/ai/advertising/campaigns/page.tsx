...
import { BarChart3, Megaphone, RefreshCw, AlertCircle, ArrowLeft } from "lucide-react";
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const filtered = (data.opportunities || []).filter((opp: Opportunity) => 
        (opp.entity_type === 'campaign' || opp.title?.toLowerCase().includes('ad') || opp.title?.toLowerCase().includes('campaign')) &&
        (opp.title?.toLowerCase().includes('campaign') ||
         opp.title?.toLowerCase().includes('performance') ||
         opp.title?.toLowerCase().includes('creative') ||
         opp.title?.toLowerCase().includes('saturation') ||
         opp.title?.toLowerCase().includes('fatigue'))
      );
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-400">
            Monitor campaign effectiveness, creative fatigue, and audience saturation
          </p>
        </div>
        ...
        <Card>
          ...
          {loading ? (
            ...
            <div className="text-center py-8 text-gray-400">
              ...
              <p>No campaign opportunities found</p>
            </div>
          ) : (
