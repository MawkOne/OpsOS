...
export default function PatternClassificationPage() {
  ...
  return (
    <AppLayout
      ...
      subtitle="Categorizes 30+ pattern types across all channels"
    >
      {/* What This Detects */}
      <Card className="glass mb-8">
        <h2 className="text-xl font-bold mb-4" style={{ color: "var(--foreground)" }}>What Pattern Classification Finds</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Accelerating Decline</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Getting worse faster each period</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Decelerating Decline</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Still declining but slowing</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Improving</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Positive momentum building</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Stable</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Consistent performance over time</p>
            </div>
          </div>
        </div>
      </Card>
