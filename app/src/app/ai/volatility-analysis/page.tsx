...
export default function VolatilityAnalysisPage() {
  ...
  async function fetchData() {
    try {
      ...
      const filtered = data.opportunities?.filter(
        (opp: any) => opp.description?.toLowerCase().includes("volatile") || opp.description?.toLowerCase().includes("unstable")
      ) || [];
  ...
  return (
    <AppLayout
      ...
      subtitle="Stability scoring, predictability, and risk assessment"
    >
      {/* What This Detects */}
      <Card className="glass mb-8">
        <h2 className="text-xl font-bold mb-4" style={{ color: "var(--foreground)" }}>What Volatility Analysis Finds</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Stability Scoring</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>How predictable is performance</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>High Volatility</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Entities with erratic performance</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Risk Assessment</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Likelihood of sudden drops</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Consistency Analysis</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Steady vs unpredictable performers</p>
            </div>
          </div>
        </div>
      </Card>
