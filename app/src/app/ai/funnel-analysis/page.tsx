...
export default function FunnelAnalysisPage() {
  ...
  async function fetchData() {
    try {
      ...
      const filtered = data.opportunities?.filter(
        (opp: any) => opp.description?.toLowerCase().includes("funnel") || opp.description?.toLowerCase().includes("conversion rate")
      ) || [];
  ...
  return (
    <AppLayout
      ...
      subtitle="Stage-to-stage conversion, drop-offs, and channel efficiency"
    >
      {/* What This Detects */}
      <Card className="glass mb-8">
        <h2 className="text-xl font-bold mb-4" style={{ color: "var(--foreground)" }}>What Funnel Analysis Finds</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Stage Conversions</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Visit → Signup → Trial → Paid rates</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Drop-off Analysis</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Where users leave the funnel</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Channel Efficiency</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Best converting traffic sources</p>
            </div>
          </div>
          <div className="flex items-start gap-3">
            <div className="w-2 h-2 rounded-full mt-2" style={{ background: "var(--accent)" }}></div>
            <div>
              <p className="font-semibold" style={{ color: "var(--foreground)" }}>Funnel Optimization</p>
              <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>Opportunities to improve flow</p>
            </div>
          </div>
        </div>
      </Card>
